...
public class DataStoreServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			out.close();
			if (retrievedCommands != null) {
				try {
					ArrayList<Command> cmds = extractCommands(retrievedCommands);
					Store.ofy().save().entities(cmds).now();
				} catch (Exception ex) {
					ex.printStackTrace();
				}
	...
	private IMCMessage dataFor(int dst, boolean remove) {
	...
	private RemoteCommand convert(Command cmd) throws IOException {
		...
		IMCInputStream iis = new IMCInputStream(new ByteArrayInputStream(cmd.cmd.getBytes()),
	...
	private void filterHistoricData(HttpServletRequest req, HttpServletResponse resp, int limit, String contenttype)
		...
		if (type != null) {
			try {
				...
				Filter typeFilter = new FilterPredicate("sample_type", FilterOperator.EQUAL, typeValue);
				list.add(typeFilter);
		...
		if (system != null) {
			try {
				...
				Filter systemFilter = new FilterPredicate("imc_id", FilterOperator.EQUAL, systemValue);
		...
		try {
			...
			Filter sinceFilter = new FilterPredicate("timestamp", FilterOperator.GREATER_THAN, date);
		...
		List<HistoricDatum> historicData = Store.ofy().load().type(HistoricDatum.class).filter(filter)
				.order("timestamp").limit(MAX_RESULTS).list();
	...
	private void listHistoricData(HttpServletRequest req, HttpServletResponse resp, int limit, String type)
			throws ServletException, IOException {

		List<HistoricDatum> historicData = Store.ofy().load().type(HistoricDatum.class).order("-timestamp").limit(limit)
				.list();
	...
	private void serveHistoricData(HistoricData data, String contentType, HttpServletResponse resp) throws IOException {
		...
		case "html":
			...
			resp.getWriter()
					.println("<h3>Initial time: " + new Date((long) ((data.getBaseTime() + t) * 1000)) + "</h3>");
