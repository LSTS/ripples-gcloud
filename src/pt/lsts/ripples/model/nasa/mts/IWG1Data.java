...
//    }
    ...
    private static void parseTimeStamp(IWG1Data ret, String tk) {
        Date dateTime = null;
        try {
            int dpt = dateTimeFormaterISO8601.toPattern().replace("'", "").length();
            String st = tk.substring(0, dpt);
            dateTime = dateTimeFormaterISO8601.parse(st);
            String st1 = dateTimeFormaterISO8601.format(dateTime);
            ...
            try {
                int dpt = dateTimeFormaterISO8601_1.toPattern().replace("'", "").length();
                String st = tk.substring(0, dpt);
                dateTime = dateTimeFormaterISO8601_1.parse(st);
                String st1 = dateTimeFormaterISO8601_1.format(dateTime);
                if (!st.equalsIgnoreCase(st1)) {
                    dateTime = null;
                    throw new ParseException("Not possible to revert parse!", 0);
                }
                ...
                try {
                    int dpt = dateTimeFormaterISO8601_2.toPattern().replace("'", "").length();
                    String st = tk.substring(0, dpt);
                    dateTime = dateTimeFormaterISO8601_2.parse(st);
                    String st1 = dateTimeFormaterISO8601_2.format(dateTime);
                    if (!st.equalsIgnoreCase(st1)) {
                        dateTime = null;
                        throw new ParseException("Not possible to revert parse!", 0);
                    }
    ...
    public String toIWG1() {
        ...
        synchronized (IWG1Data.class) {
            ...
            if (timeStampMillis >= 0) {
                sb.append(dateTimeFormaterISO8601.format(new Date(new Double(timeStampMillis).longValue())));
            }
            sb.append(",");
            if (!Double.isNaN(latitudeDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees180(latitudeDegs), 7));
            }
            sb.append(",");
            if (!Double.isNaN(longitudeDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees180(longitudeDegs), 7));
            }
            sb.append(",");
            if (!Double.isNaN(gpsAltMSL)) {
                sb.append(MathUtils.round(gpsAltMSL, 2));
            }
            sb.append(",");
            if (!Double.isNaN(gpsAltitude)) {
                sb.append(MathUtils.round(gpsAltitude, 2));
            }
            sb.append(",");
            if (!Double.isNaN(pressureAltitude)) {
                sb.append(MathUtils.round(pressureAltitude, 2));
            }
            sb.append(",");
            if (!Double.isNaN(radarAltitude)) {
                sb.append(MathUtils.round(radarAltitude, 2));
            }
            sb.append(",");
            if (!Double.isNaN(groundSpeed)) {
                sb.append(MathUtils.round(groundSpeed, 2));
            }
            sb.append(",");
            if (!Double.isNaN(trueAirSpeed)) {
                sb.append(MathUtils.round(trueAirSpeed, 2));
            }
            sb.append(",");
            if (!Double.isNaN(indicatedAirSpeed)) {
                sb.append(MathUtils.round(indicatedAirSpeed, 2));
            }
            sb.append(",");
            if (!Double.isNaN(machNumber)) {
                sb.append(MathUtils.round(machNumber, 2));
            }
            sb.append(",");
            if (!Double.isNaN(verticalSpeed)) {
                sb.append(MathUtils.round(verticalSpeed, 2));
            }
            sb.append(",");
            if (!Double.isNaN(trueHeadingDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees360(trueHeadingDegs), 2));
            }
            sb.append(",");
            if (!Double.isNaN(trackAngleDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees360(trackAngleDegs), 2));
            }
            sb.append(",");
            if (!Double.isNaN(driftAngleDegs)) {
                sb.append(MathUtils.round(driftAngleDegs, 2));
            }
            sb.append(",");
            if (!Double.isNaN(pitchAngleDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees180(pitchAngleDegs), 2));
            }
            sb.append(",");
            if (!Double.isNaN(rollAngleDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees180(rollAngleDegs), 2));
            }
            sb.append(",");
            if (!Double.isNaN(slipAngleDegs)) {
                sb.append(MathUtils.round(slipAngleDegs, 2));
            }
            sb.append(",");
            if (!Double.isNaN(attackAngleDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees180(attackAngleDegs), 2));
            }
            sb.append(",");
            if (!Double.isNaN(staticAirTemp)) {
                sb.append(MathUtils.round(staticAirTemp, 1));
            }
            sb.append(",");
            if (!Double.isNaN(dewPoint)) {
                sb.append(MathUtils.round(dewPoint, 1));
            }
            sb.append(",");
            if (!Double.isNaN(totalAirTemp)) {
                sb.append(MathUtils.round(totalAirTemp, 1));
            }
            sb.append(",");
            if (!Double.isNaN(staticPressure)) {
                sb.append(MathUtils.round(staticPressure, 2));
            }
            sb.append(",");
            if (!Double.isNaN(dynamicPressure)) {
                sb.append(MathUtils.round(dynamicPressure, 2));
            }
            sb.append(",");
            if (!Double.isNaN(cabinPressure)) {
                sb.append(MathUtils.round(cabinPressure, 2));
            }
            sb.append(",");
            if (!Double.isNaN(windSpeed)) {
                sb.append(MathUtils.round(windSpeed, 2));
            }
            sb.append(",");
            if (!Double.isNaN(windDirection)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees360(windDirection), 2));
            }
            sb.append(",");
            if (!Double.isNaN(vertWindSpeed)) {
                sb.append(MathUtils.round(vertWindSpeed, 2));
            }
            sb.append(",");
            if (!Double.isNaN(solarZenithAngleDegs)) {
                sb.append(MathUtils.round(solarZenithAngleDegs, 2));
            }
            sb.append(",");
            if (!Double.isNaN(aircraftSunElevationDegs)) {
                sb.append(MathUtils.round(aircraftSunElevationDegs, 2));
            }
            sb.append(",");
            if (!Double.isNaN(sunAzimuthDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees360(sunAzimuthDegs), 2));
            }
            sb.append(",");
            if (!Double.isNaN(aircraftSunAzimuthDegs)) {
                sb.append(MathUtils.round(MathUtils.nomalizeAngleDegrees360(aircraftSunAzimuthDegs), 2));
            }
            ...
            if (!Double.isNaN(bathymetry)) {
                sb.append(MathUtils.round(bathymetry, 2));
            }
            sb.append(",");
            if (!Double.isNaN(temperature)) {
                sb.append(MathUtils.round(temperature, 1));
            }
            sb.append(",");
            if (!Double.isNaN(salinity)) {
                sb.append(MathUtils.round(salinity, 2));
            }
            sb.append(",");
            if (!Double.isNaN(condutivity)) {
                sb.append(MathUtils.round(condutivity, 2));
            }
        }
        sb.append("\r\n");
        return sb.toString();
